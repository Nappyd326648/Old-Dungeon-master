...
import com.old.olddungeon.items.ArcaneResin;
import com.old.olddungeon.items.Generator;
import com.old.olddungeon.items.Item;
import com.old.olddungeon.items.LiquidMetal;
import com.old.olddungeon.items.Recipe;
import com.old.olddungeon.items.bombs.Bomb;
import com.old.olddungeon.items.food.Blandfruit;
import com.old.olddungeon.items.food.Food;
import com.old.olddungeon.items.food.MeatPie;
import com.old.olddungeon.items.food.MysteryMeat;
import com.old.olddungeon.items.food.Pasty;
import com.old.olddungeon.items.food.StewedMeat;
import com.old.olddungeon.items.potions.AlchemicalCatalyst;
import com.old.olddungeon.items.potions.Potion;
import com.old.olddungeon.items.potions.brews.BlizzardBrew;
import com.old.olddungeon.items.potions.brews.CausticBrew;
import com.old.olddungeon.items.potions.brews.InfernalBrew;
import com.old.olddungeon.items.potions.brews.ShockingBrew;
import com.old.olddungeon.items.potions.elixirs.ElixirOfAquaticRejuvenation;
import com.old.olddungeon.items.potions.elixirs.ElixirOfArcaneArmor;
import com.old.olddungeon.items.potions.elixirs.ElixirOfDragonsBlood;
import com.old.olddungeon.items.potions.elixirs.ElixirOfHoneyedHealing;
import com.old.olddungeon.items.potions.elixirs.ElixirOfIcyTouch;
import com.old.olddungeon.items.potions.elixirs.ElixirOfMight;
import com.old.olddungeon.items.potions.elixirs.ElixirOfToxicEssence;
import com.old.olddungeon.items.potions.exotic.ExoticPotion;
import com.old.olddungeon.items.scrolls.Scroll;
import com.old.olddungeon.items.scrolls.exotic.ExoticScroll;
import com.old.olddungeon.items.spells.Alchemize;
import com.old.olddungeon.items.spells.AquaBlast;
import com.old.olddungeon.items.spells.ArcaneCatalyst;
import com.old.olddungeon.items.spells.BeaconOfReturning;
import com.old.olddungeon.items.spells.CurseInfusion;
import com.old.olddungeon.items.spells.FeatherFall;
import com.old.olddungeon.items.spells.MagicalInfusion;
import com.old.olddungeon.items.spells.PhaseShift;
import com.old.olddungeon.items.spells.ReclaimTrap;
import com.old.olddungeon.items.spells.Recycle;
import com.old.olddungeon.items.spells.SummonElemental;
import com.old.olddungeon.items.spells.TelekineticGrab;
import com.old.olddungeon.items.spells.WildEnergy;
...
public class QuickRecipe extends Component {
	...
	public QuickRecipe(Recipe r, ArrayList<Item> inputs, final Item output) {
		...
		for (final Item in : inputs) {
			...
			curr = new ItemSlot(in) {
				...
				protected void onClick() {
					OldDungeon.scene().addToFront(new WndInfoItem(in));
				}
			};
		...
		if (hasInputs) {
			...
			if (!(OldDungeon.scene() instanceof AlchemyScene)) {
				arrow.enable(false);
			}
		} else {
		...
		this.output = new ItemSlot(output){
			@Override
			protected void onClick() {
				OldDungeon.scene().addToFront(new WndInfoItem(output));
			}
		};
		if (!hasInputs){
	...
	public class arrow extends IconButton {
		...
		protected void onClick() {
			...
			((AlchemyScene) OldDungeon.scene()).populate(ingredients, Dungeon.hero.belongings);
		}
