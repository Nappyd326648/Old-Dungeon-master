...
import com.old.olddungeon.actors.hero.HeroClass;
...
import com.old.olddungeon.ui.ExitButton;
import com.old.olddungeon.ui.IconButton;
...
import com.old.olddungeon.windows.WndTextInput;
import com.old.olddungeon.utils.DungeonSeed;
import com.old.olddungeon.windows.WndChallenges;
import com.old.olddungeon.windows.WndHeroInfo;
...
public class HeroSelectScene extends PixelScene {
	...
	protected void onBackPressed() {
		if (btnExit.active){
			OldDungeon.switchScene(TitleScene.class);
		} else {
			super.onBackPressed();
		}
	}
	...
	private class HeroBtn extends StyledButton {
		...
		protected void onClick() {
			...
			if( !cl.isUnlocked() ){
				OldDungeon.scene().addToFront( new WndMessage(cl.unlockMsg()));
			...
			if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)){
				StyledButton seedButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "custom_seed"), 6){
					...
					protected void onClick() {
						...
						OldDungeon.scene().addToFront(new WndTextInput(Messages.get(HeroSelectScene.class, "custom_seed_title"),
								...
								if (positive && seed != -1){
									...
									for (GamesInProgress.Info info : GamesInProgress.checkAll()){
										if (info.customSeed.isEmpty() && info.seed == seed){
											SPDSettings.customSeed("");
											icon.resetColor();
											OldDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "custom_seed_duplicate")));
											return;
										}
									}
				...
				StyledButton dailyButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "daily"), 6){
					...
					protected void onClick() {
						...
						if (diff > 24*HOUR){
							OldDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_unavailable_long", (diff / DAY)+1)));
							return;
						}

						for (GamesInProgress.Info game : GamesInProgress.checkAll()){
							if (game.daily){
								OldDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_existing")));
								return;
							}
						}
						...
						OldDungeon.scene().addToFront(new WndOptions(
								icon,
				...
				StyledButton challengeButton = new StyledButton(Chrome.Type.BLANK, Messages.get(WndChallenges.class, "title"), 6){
					@Override
					protected void onClick() {
						OldDungeon.scene().addToFront(new WndChallenges(SPDSettings.challenges(), true) {
